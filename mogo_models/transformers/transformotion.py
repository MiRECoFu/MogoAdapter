...
names = [
    "Hips", "Left_UpperLeg", "Right_UpperLeg", "Spine", "Left_LowerLeg", "Right_LowerLeg",
        "Chest", "Left_Foot", "Right_Foot", "UpperChest", "Left_Toes", "Right_Toes",
        "Neck", "Left_Shoulder", "Right_Shoulder", "Head", "Left_UpperArm", "Right_UpperArm",
        "Left_LowerArm", "Right_LowerArm", "Left_Hand", "Right_Hand"
]
re_order = [0, 1, 4, 7, 10, 2, 5, 8, 11, 3, 6, 9, 12, 15, 13, 16, 18, 20, 14, 17, 19, 21]

sorted_names = [names[i] for i in re_order]
...
class Transformotion(nn.Module):
    ...
    def forward(self, prompt_texts, motion_ids, m_lens, labels, mems=tuple(), is_generating=False, has_adapter=False):
        ...
        for ind, transformer in enumerate(self.xls):
            ...
            *prec_dims, _, _ = stage_tokens.shape
            ...
            all_attends.append(attended)
        out = torch.stack(all_logits, dim=-1)
        out = out[:, 1:, :, :] if is_generating==False else out
                
        all_attends_out = torch.stack(all_attends, dim=-1)
        all_attends_out = all_attends_out[:, 1:, :, :]
        ...
        if is_generating is False:
            ...
            if has_adapter is True:
                return ce_loss, acc, pred_id, out, logits, all_attends_out
