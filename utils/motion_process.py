...
def recover_from_ric(data, joints_num):
    r_rot_quat, r_pos = recover_root_rot_pos(data)
    # print(f"r_rot_quat,{r_rot_quat}, {r_rot_quat.shape}")
    positions = data[..., 4:(joints_num - 1) * 3 + 4]
    positions = positions.view(positions.shape[:-1] + (-1, 3))

    '''Add Y-axis rotation to local joints'''
    positions = qrot(qinv(r_rot_quat[..., None, :]).expand(positions.shape[:-1] + (4,)), positions)
    expanded_qinv = qinv(r_rot_quat[..., None, :]).expand(positions.shape[:-1] + (4,))
    # print(f"qinv(r_rot_quat[..., None, :]).expand(positions.shape[:-1] + (4,)),{expanded_qinv}")
    # np.save('test.npy', expanded_qinv.cpu().numpy())
